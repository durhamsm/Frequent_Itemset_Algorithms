...
itemset_list_t get_itemset_list_1_item() {
	int item_num, trans_id, support_count;
...
//				append_to_trans_ids_list(&(itemset_list_item->trans_ids_list), trans_id);
		...
		if (support_count >= MIN_SUP) {
			itemset_list_item->support = support_count;
...
//not needed after removed trans ids list (I don't think it was even being used before)
...
//Not needed after remove trans_ids_list from itemset item
...
int is_transaction_a_subset(transaction_t potential_subset_transaction, transaction_t super_set_transaction) {
	int item_num;

	for (item_num = 0; item_num < NUM_ITEMS; ++item_num) {

		if (potential_subset_transaction[item_num] && !super_set_transaction[item_num]) {
			return 0;
		}
	}
...
int get_support_count(transaction_t transaction) {
	int support_count, trans_id;
	support_count = 0;

	for (trans_id = 0; trans_id < NUM_TRANS; ++trans_id) {
		if (is_transaction_a_subset(transaction, dataset.transactions[trans_id])) {
			++support_count;
		}
	}
...
//	trans_ids_list_t trans_ids_list = get_null_trans_ids_list();
	...
	support_count = get_support_count(transaction);

//	fill_trans_ids_from_itemsets(lhs_item, rhs_item, &trans_ids_list);

	new_itemset_list_item = create_new_itemset_list_item(NULL, transaction, prefix_id);
	new_itemset_list_item->last_item_id = rhs_item->last_item_id;
	new_itemset_list_item->support = support_count;
...
void print_itemset_list_set(itemset_lists_set_t* itemset_list_set) {
	...
	for (set_level = 0; set_level <= NUM_ITEMS; ++set_level) {
		if (itemset_list_set->level_lists[set_level].num_elements > 0) {
			printf("Set Level: %d\n", set_level);
...
//	itemset_list_item_t* new_itemset_list_item = allocate_null_itemset_list_item();
	...
	//Not needed after removing trans_ids_list
//	new_itemset_list_item->trans_ids_list = deep_copy_trans_ids_list(item_to_copy->trans_ids_list);
	...
	new_itemset_list_item->support = item_to_copy->support;
...
void deallocate_transactions(transaction_t* transactions, int num_transactions) {
	int trans_index;
	//FIXME
	for (trans_index = 0; trans_index < num_transactions; ++trans_index) {
		free(transactions[trans_index]);
	}
...
void deallocate_dataset(dataset_t* dataset) {
	deallocate_transactions(dataset->transactions, dataset->params.num_trans);
}
...
void apriori_generate_frequent_itemsets(enum APRIORI_ANALYSIS_TYPE analysis_type) {
	...
	for (num_items_current_itemsets = 2; num_items_current_itemsets <= NUM_ITEMS; ++num_items_current_itemsets) {
		printf("Begin frequent itemset generation for %d-item itemsets\n", num_items_current_itemsets);
